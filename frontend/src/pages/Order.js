                ...
                for(const itm of item){
                    await fetch('http://localhost:5000/vendor',{
                                ...
                                headers:{
                                        'Authorization': `Bearer ${token}`,
                                        'content-type':'application/json'},
                                body:JSON.stringify({vendor_id:itm.created_by, item_id:itm.id, item_details:[itm], order_id:orderId, quantity:itm.quantity, status:status, client_id:userId, client_details:formData})
                            });
                }
