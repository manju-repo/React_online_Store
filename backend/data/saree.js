...
async function getAll() {
  const fabrics=await Fabric.find({category:'sarees'});
...
async function getCollection(sub_category) {
  ...
  const fabrics=await Fabric.find({category:'sarees',sub_category:sub_category});
...
//console.log(req.body);
    ...
    const {category, sub_category, type, image, desc, colour, colour_options, price, stock, details, created_by} = req.body;
    //console.log(category, sub_category, type, image, desc, colour, colour_options, price, details, created_by);
...
const id=req.params.id;
    if (req.userData.userType!=='admin' && req.userData.userId!==created_by){
       return res.status(403).json({success:false,message:'You are not authorized to edit items'});
    }
    //console.log("from data PUT "+id, category,sub_category, type, image, desc, colour, colour_options, price, stock, details, created_by);
   try{
        const updatedItem=await Fabric.findByIdAndUpdate(id,{category:category, sub_category:sub_category, type:type, desc:desc,
        price:price, image:image, colour:colour, colour_options:colour_options, stock:stock, details:details});
